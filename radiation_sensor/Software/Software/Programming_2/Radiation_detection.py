...
def plotData(x,y):
    ...
    try:
        ...
        plt.savefig('Radiation_Measurement.png')
...
def peakFinder():
    """ The following prgram runs the Fortran Peak detection algorithm"""
    
    try:
        sp.call('detection.exe')
        
    except Exception:
        print('Error in peak finding program')
        
def radDetermination(filename):
    """The following function determines heightened Radiation"""
    try:
        peaker = np.loadtxt(filename, usecols=[0], dtype=int)
        energy = np.loadtxt(filename, usecols=[1], dtype=float)
        
        peakcount = peaker.sum()
        
        if peakcount > 200:
            print('HEIGHTENED RADIATION PRESENT')       
		
    except Exception:
        print('error in peaker')
...
#runSorter()
# # The following line runs the Fortran peak-finder program
peakFinder()
...
radDetermination('PEAK_DATA.TXT')
